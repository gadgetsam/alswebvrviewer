...
window.addEventListener('DOMContentLoaded', function(){
  ...
  vrHelper.webVRCamera.minZ = 0;
      vrHelper.raySelectionPredicate = (mesh) => {return mesh.isPickable}
  var spheres = []
...
scene.registerBeforeRender(function () {
  ...
  // var newInstance = sphere.createInstance("i"+counter);
             var m = new BABYLON.Matrix(); mesh2.getWorldMatrix().invertToRef(m); var v = BABYLON.Vector3.TransformCoordinates(webVRCamera.rightController.devicePosition, m);
...
//       disc.material = myMaterial
  ...
  for(var x=1; x<numImages; x++){ 
    ...
    var mainTexture = new BABYLON.Texture("https://storage.googleapis.com/webxr/shoulder/Adam.to-byte"+pad(x,3)+".png", scene);
  ...
  var box = BABYLON.MeshBuilder.CreatePlane("box", {}, scene);
  box.material = shaderMaterial;
  box.setPositionWithLocalVector(new BABYLON.Vector3(0,0,x/480));
    ...
    plane.position = new BABYLON.Vector3(0.1, 1, 0.1)
    ...
    console.log(webVRController.hand + " controller ready.");
          if(webVRController.hand == "right"){
            webVRCamera.rightController.onMainButtonStateChangedObservable.add(function (stateObject) {
...
console.log("Test")
                  ...
                  webVRCamera.leftController.onXButtonStateChangedObservable.add(function(stateObject) {
        ...
        webVRCamera.leftController.onYButtonStateChangedObservable.add(function(stateObject) {
      ...
      moveMesh.dispose();
            console.log("test2")
    webVRCamera.leftController.onSecondaryTriggerStateChangedObservable.add(function (stateObject) {
